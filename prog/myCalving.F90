...
!------------------------------------------------------------------------------
   ...
   IF(ParEnv % Pes > 1) CALL Fatal(SolverName, "Solver doesn't work in parallel!")
   ...
   IF(FirstTime .OR. Solver % Mesh % Changed) THEN
      IF (.NOT.FirstTime) &
         DEALLOCATE(Vel1InvPerm,NodeNeighbours,NumNeighbours)
      
      FirstTime = .FALSE.

      !STRATEGY: Finding neighbours on the fly works fine UNLESS you are in a recursive subroutine
      !Then it messes up, because at each level, ThisNodeNeighbours is deallocated and reallocated,
   ...
   DO i = 1, NoNodes
     IF(Permutation(i) == 0) CYCLE
       NodeLength = Model % Nodes % x(i) ! Finds the maximum value of Coordinate 1 
