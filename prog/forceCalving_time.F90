...
!------------------------------------------------------------------------------
   ...
   REAL (KIND=dp) :: t, CalvingSize, CalvingSizeTemp, SteadyStateLength, xcoord, ycoord, NodeDepth, Length, &
   ...
   !CalvingSizeTemp = GetCReal(SolverParams,'Calving Size',Found )
   !SteadyStateLength = GetCReal(SolverParams,'Length',Found )
   !IF(Length .GT. (SteadyStateLength + CalvingSizeTemp/2)) THEN
   !   ! Get Calving Size from solver parameters
   !   CalvingSize = GetCReal(SolverParams,'Calving Size',Found )
      !CalvingSize = 100.0_dp
   !ELSE
   !   CalvingSize = 0.0_dp
   !END IF

   ! 2 in mod refers to two days...
   PRINT *, '**** t = ', t
   PRINT *, '**** t/dt  = ', t/dt
   PRINT *, '**** mod(t/dt,4) = ', floor(mod(t/dt,20.0_dp))
   IF(floor(mod(t/dt,20.0_dp)) .EQ. 1) THEN
      CalvingSize = GetCReal(SolverParams,'Calving Size',Found)
   ELSE
      CalvingSize = 0.0_dp
   END IF
